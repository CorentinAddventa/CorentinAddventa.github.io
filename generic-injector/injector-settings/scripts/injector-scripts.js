...
function buildFormStd()
{
	...
	body.append("<form name='" + _formId + "' id='" + _formId + "'>" +
		...
		$("#propertiesButtons").append(
						'<a href="#" id="buildXML" onclick="buildXML();" class="ui-state-default ui-corner-all button">' +
	...
	if (caseGroups)
	{
		...
		$.each(caseGroups, function(i, _caseGroup)
		{
			...
			{
				caseGroupFS.prepend('<h5>' +  _caseGroup.title.replace(/\s/g, "") +'<i class="fa fa-chevron-up"></i></h5>');
			}
...
function buildFormSamePage()
{
	...
	$("#launch").click(function() {generateResult();});
	 ...
	 addCss("injector-settings/css/perfect-scrollbar.css");
