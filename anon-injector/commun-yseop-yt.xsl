...
-->
	...
	<xsl:template match="yt:bold">
		<b>
			<xsl:choose>
			 	<xsl:when test="contains(text(), 'Allianz')">investors</xsl:when>
				<xsl:otherwise>
					<xsl:apply-templates/>
				</xsl:otherwise>
			</xsl:choose>
	...
	<xsl:template match="yt:par">
		<p>
			...
			<xsl:choose>
			 	<xsl:when test="contains(text(), 'Concentra')">
			 		Germania
				</xsl:when>
				<xsl:when test="contains(text(), 'Allianz Global Sustainability')">
	...
	<xsl:template match="yt:picture">
		<xsl:choose>
			<xsl:when test="@src = '../images/agi_l_blue_rgb.png'">
			</xsl:when>
			<xsl:otherwise>
				<img>
